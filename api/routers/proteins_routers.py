...
def add_protein(item: ItemIn, response: Response, account_data:
...
def get_all_proteins(account_data: dict =
    ...
    return repo.get_all_proteins(account_id=account_data['id'])


@router.put("/proteins/{item_id}", response_model=Union[ItemOut, Error])
def update_protein(item_id: str, item: ItemIn, account_data: dict =
                   Depends(authenticator.get_current_account_data),
...
def delete_protein(item_id: str, account_data: dict =
                   Depends(authenticator.get_current_account_data),
...
def get_protein(item_id: str, response: Response, account_data: dict =
                Depends(authenticator.get_current_account_data),
