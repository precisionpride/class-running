...
def add_produce(item: ItemIn, response: Response, account_data: dict =
...
def get_all_produce(account_data: dict =
                    Depends(authenticator.get_current_account_data),
                    repo: ItemRepository = Depends()):
    return repo.get_all_produce(account_id=account_data['id'])


@router.put("/produce/{item_id}", response_model=Union[ItemOut, Error])
def update_produce(item_id: str, item: ItemIn, account_data: dict =
                   Depends(authenticator.get_current_account_data),
                   repo: ItemRepository = Depends()) -> Union[Error, ItemOut]:
...
def delete_produce(item_id: str, account_data: dict =
                   Depends(authenticator.get_current_account_data),
...
def get_produce(item_id: str, response: Response, account_data: dict =
                Depends(authenticator.get_current_account_data),
