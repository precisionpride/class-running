...
class ItemRepository(MongoQueries):
    ...
    def get_grain(self, item_id: str,
                  account_id: str) -> Optional[GrainItemOut]:
        grains_queries = MongoQueries(collection_name="grains")
        record = grains_queries.collection.find_one(
    ...
    def delete_grain(self, item_id: str, account_id: str) -> bool:
        grains_queries = MongoQueries(collection_name="grains")
        result = grains_queries.collection.delete_one(
            {"_id": ObjectId(item_id), "account_id": account_id})
        return result.deleted_count > 0


    def get_all_grains(self, account_id:
                            str) -> Union[Error, List[GrainItemOut]]:
        grains_queries = MongoQueries(collection_name="grains")
        try:
            records = grains_queries.collection.find(
